...
async function ensureDailyQuestionsGenerated(user, pool) {
    ...
    try {
        ...
        const newQuestionIds = await generateDailyQuestionsForUser(userLevel, pool);
...
passport.use(new GoogleStrategy({
    ...
    async (accessToken, refreshToken, profile, done) => {
        ...
        try {
            ...
            let [users] = await conn.query('SELECT * FROM users WHERE google_id = ?', [id]);
            let user = users[0];
            ...
            if (!user) {
                ...
                await conn.query('UPDATE users SET last_login = ? WHERE user_id = ?', [new Date(), user.user_id]);
            }
...
app.get('/login/callback',
    passport.authenticate('google', { failureRedirect: `${VITE_FRONTEND_URL}/?auth=failed` }),
    async (req, res) => {
        await ensureDailyQuestionsGenerated(req.user, dbPool);
        res.redirect(`${VITE_FRONTEND_URL}/practice`);
    }
);
...
app.get('/api/auth/verify/:token', async (req, res) => {
    ...
    try {
        const [users] = await dbPool.query('SELECT * FROM users WHERE verification_token = ?', [token]);
        if (users.length === 0) return res.status(400).json({ error: 'Invalid token' });

        await dbPool.query('UPDATE users SET is_verified = true, verification_token = NULL WHERE verification_token = ?', [token]);
